...
.DESCRIPTION
    ...
    lab-specific baseline, and installs/verifies the required analysis tools.
    ...
    This setup script does not:
      ...
      - install or reconfigure the Sysmon service;
...
try {
    ...
    if ($downloadedFiles.ContainsKey("sysmon_uboatrat.xml")) {
        Assert-SysmonXml `
    ...
    # ------------------------------------------------------------------------
    # 6. Install/verify analysis tools without upgrading valid existing copies
    # ------------------------------------------------------------------------
    ...
    if ($null -eq $sysmonService) {
        Write-WarningMessage (
            "Sysmon is not installed as a service. " +
            "Sysmon64.exe was only placed under C:\Tools\Sysmon."
        )
    }
    else {
        Write-Success (
            "Existing Sysmon service detected: " +
            "$($sysmonService.Name) [$($sysmonService.Status)]."
        )

        Write-Info (
            "The setup did not install, update, or replace its configuration."
        )
    }

    if (-not $downloadedFiles.ContainsKey("sysmon_uboatrat.xml")) {
        Write-WarningMessage (
            "sysmon_uboatrat.xml was not downloaded because it is not " +
            "present in Git yet. Add it before the final VM snapshot."
        )
    }
    ...
    Write-Host ""
    Write-Host "Take the Windows VM snapshot only after " `
        -NoNewline `
        -ForegroundColor Cyan
    Write-Host "sysmon_uboatrat.xml" `
        -NoNewline `
        -ForegroundColor White
    Write-Host " is added and validated." `
        -ForegroundColor Cyan
}
catch {
    $setupError = $_
